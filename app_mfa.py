...
def login():
    ...
    "Risk Score"
        ...
        feature_data = {feature: flow_features.get(feature, 0) for feature in selected_features}  
        df_features = pd.DataFrame([feature_data])
        df_features["Risk Score"] = risk_score
        df_features["Attack Severity"] = predicted_label
        print(df_features)
        ...
        with open(LOG_FILE, 'a', newline='') as f:
            ...
            if login_status == 'Success':
                ...
                session['require_totp'] = bool(predicted_label == 2)
                if session['require_totp'] and not session.get('totp_secret'):
                    return redirect('/mfa/totp/setup')
